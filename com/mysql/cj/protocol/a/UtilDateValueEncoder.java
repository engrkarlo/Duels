...
public class UtilDateValueEncoder
extends AbstractValueEncoder {
    ...
    public String getString(BindValue binding) {
        ...
        Timestamp x = this.adjustTimestamp(new Timestamp(((java.util.Date)binding.getValue()).getTime()), binding.getField(), binding.keepOrigNanos());
        switch (binding.getMysqlType()) {
            case DATE: {
                return binding.getCalendar() != null ? TimeUtil.getSimpleDateFormat("''yyyy-MM-dd''", binding.getCalendar()).format(new Date(((java.util.Date)binding.getValue()).getTime())) : TimeUtil.getSimpleDateFormat(null, "''yyyy-MM-dd''", this.serverSession.getDefaultTimeZone()).format(new Date(((java.util.Date)binding.getValue()).getTime()));
            }
            case DATETIME: 
            case TIMESTAMP: 
            case CHAR: 
            ...
            case LONGTEXT: {
                ...
                if (binding.getCalendar() != null) {
                    ...
                    this.tsdf = TimeUtil.getSimpleDateFormat(this.tsdf, "''yyyy-MM-dd HH:mm:ss", binding.getMysqlType() == MysqlType.TIMESTAMP && (Boolean)this.preserveInstants.getValue() != false ? this.serverSession.getSessionTimeZone() : this.serverSession.getDefaultTimeZone());
            ...
            case TIME: {
                ...
                sb.append(this.adjustLocalTime(new Timestamp(((java.util.Date)binding.getValue()).getTime()).toLocalDateTime().toLocalTime(), binding.getField()).format(TimeUtil.TIME_FORMATTER_WITH_OPTIONAL_MICROS));
                sb.append("'");
                return sb.toString();
            }
        }
    ...
    public void encodeAsBinary(Message msg, BindValue binding) {
        ...
        switch (binding.getMysqlType()) {
            ...
            case TIME: {
                ...
                calendar.setTime(t);
                this.writeTime(msg, InternalTime.from(calendar, this.adjustTimestamp(new Timestamp(((java.util.Date)binding.getValue()).getTime()), binding.getField(), binding.keepOrigNanos()).getNanos()));
                return;
            }
            case CHAR: 
            case VARCHAR: 
