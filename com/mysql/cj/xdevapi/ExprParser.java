...
public class ExprParser {
    ...
    void lex() {
        block31: for (int i = 0; i < this.string.length(); ++i) {
            int start = i;
            char c = this.string.charAt(i);
                ...
                switch (c) {
                    ...
                    case '`': {
                        ...
                        try {
                            ...
                            while (c != quoteChar || escapeNextChar || i + 1 < this.string.length() && this.string.charAt(i + 1) == quoteChar) {
                                if (escapeNextChar) {
    ...
    void assertTokenAt(int pos, TokenType type) {
        if (this.tokens.size() <= pos) {
            throw new WrongArgumentException("No more tokens when expecting " + (Object)((Object)type) + " at token position " + pos);
        }
        if (this.tokens.get((int)pos).type != type) {
            throw new WrongArgumentException("Expected token type " + (Object)((Object)type) + " at token position " + pos);
        }
    }
