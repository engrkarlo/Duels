...
public class FAWESchematicAdapter
implements ISchematicAdapter {
    private final UltimateDuels plugin;
    private final File schematicsFolder;

    public FAWESchematicAdapter(@Nonnull UltimateDuels plugin, @Nonnull File schematicsFolder) {
        this.plugin = plugin;
    ...
    public CompletableFuture<Boolean> paste(@Nonnull String name, @Nonnull Location origin) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                try (EditSession editSession = WorldEdit.getInstance().newEditSession(world);){
                    Operation operation = new ClipboardHolder(clipboard).createPaste((Extent)editSession).to(BlockVector3.at((int)origin.getBlockX(), (int)origin.getBlockY(), (int)origin.getBlockZ())).ignoreAirBlocks(false).build();
                    Operations.complete((Operation)operation);
                }
                return true;
            }
            catch (Exception e) {
                this.plugin.getLogger().log(Level.SEVERE, "Failed to paste schematic: " + name, e);
                return false;
            }
        });
    }
