                ...
                if (this.paramInfo != null && !this.paramInfo.fakeParameters && q.getParameterCount() >= 0) {
                    ...
                    if (startPos != -1 && (parenOpenPos = q.getOriginalSql().indexOf(40, startPos + 4)) != -1 && (parenClosePos = StringUtils.indexOfIgnoreCase(parenOpenPos, q.getOriginalSql(), ")", "'\"", "'\"", SearchMode.__FULL)) != -1) {
                        ...
                        if (parsedParametersCount > parameterCountFromMetaData) {
                            throw SQLError.createSQLException(Messages.getString("CallableStatement.12", new Object[]{q.getParameterCount(), parameterCountFromMetaData}), "S1009", this.getExceptionInterceptor());
                        }
                        int placeholderCount = startIndex;
                        for (int i = 0; i < parsedParametersCount; ++i) {
                            String param = parsedParameters.get(i);
                            long questionMarkCount = 0L;
                            questionMarkCount = "?".equals(param) ? 1L : StringUtils.stripCommentsAndHints(param, "'\"", "'\"", !this.session.getServerSession().isNoBackslashEscapesSet()).codePoints().filter(c -> c == 63).count();
                            int j = 0;
                            while ((long)j < questionMarkCount) {
        ...
        try {
            ...
            synchronized (object) {
                ...
                try {
                    ...
                    boolean dbIsSchema = this.session.getPropertySet().getEnumProperty(PropertyKey.databaseTerm).getValue() == PropertyDefinitions.DatabaseTerm.SCHEMA;
                    DatabaseMetaData dbmd = this.connection.getMetaData();
                    if (this.callingStoredFunction) {
                        paramTypesRs = dbIsSchema ? dbmd.getFunctionColumns(null, useDb ? this.getCurrentDatabase() : tmpDb, procName, "%") : dbmd.getFunctionColumns(useDb ? this.getCurrentDatabase() : tmpDb, null, procName, "%");
                    } else {
                        ...
                        try {
                            ...
                            paramTypesRs = dbIsSchema ? dbmd.getProcedureColumns(null, useDb ? this.getCurrentDatabase() : tmpDb, procName, "%") : dbmd.getProcedureColumns(useDb ? this.getCurrentDatabase() : tmpDb, null, procName, "%");
                        }
                        finally {
    ...
    implements ParameterMetaData {
        ...
        protected void checkBounds(int paramIndex) throws SQLException {
            ...
            if (paramIndex < 0 || localParamIndex >= ((PreparedQuery)CallableStatement.this.query).getParameterCount()) {
                throw SQLError.createSQLException(Messages.getString("CallableStatement.11", new Object[]{paramIndex, ((PreparedQuery)CallableStatement.this.query).getParameterCount()}), "S1009", CallableStatement.this.getExceptionInterceptor());
            }
        }
