...
def player_state(text):
    # The CFR patch script may be run more than once; keep only one field/initializer.
    text = re.sub(
        r'(    private final Map<UUID, PlayerState> lobbyStates;\n)(?:\1)+',
        r'\1', text
    )
    text = re.sub(
        r'(        this\.lobbyStates = new ConcurrentHashMap<UUID, PlayerState>\(\);\n)(?:\1)+',
        r'\1', text
    )
...
def lobby(text):
    ...
    text = text.replace(
        ...
        'pdc.set(this.lobbyItemKey, PersistentDataType.BYTE, (byte) 1);'
    )
    return text
...
def ffa(text):
    ...
    text = text.replace(
        ...
        'message.append(String.join(" §7| ", rewardsList.stream().map(String::valueOf).toList()));'
    )
    return text
