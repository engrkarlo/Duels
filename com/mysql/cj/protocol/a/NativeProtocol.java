...
import com.mysql.cj.protocol.a.NullValueEncoder;
...
RuntimeProperty.RuntimePropertyListener {
    ...
    public Supplier<ValueEncoder> getValueEncoderSupplier(Object obj) {
        ...
        if (res == null && (mysqlType = DEFAULT_ENCODERS.entrySet().stream().filter(m -> ((Class)m.getKey()).isAssignableFrom(obj.getClass())).map(Map.Entry::getValue).findFirst()).isPresent()) {
            res = mysqlType.get();
        }
        return res;
    }
    ...
    public final <T> T readServerStatusForResultSets(NativePacketPayload rowPacket, boolean saveOldStatus) {
        ...
        if (rowPacket.isEOFPacket()) {
            ...
            OkPacket ok;
            result = ok = OkPacket.parse(rowPacket, this.serverSession);
