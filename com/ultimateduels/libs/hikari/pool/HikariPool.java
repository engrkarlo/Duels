...
import com.ultimateduels.libs.hikari.HikariPoolMXBean;
import com.ultimateduels.libs.hikari.metrics.MetricsTrackerFactory;
import com.ultimateduels.libs.hikari.metrics.PoolStats;
import com.ultimateduels.libs.hikari.metrics.dropwizard.CodahaleHealthChecker;
import com.ultimateduels.libs.hikari.metrics.dropwizard.CodahaleMetricsTrackerFactory;
import com.ultimateduels.libs.hikari.metrics.micrometer.MicrometerMetricsTrackerFactory;
import com.ultimateduels.libs.hikari.pool.PoolBase;
import com.ultimateduels.libs.hikari.pool.PoolEntry;
import com.ultimateduels.libs.hikari.pool.ProxyConnection;
import com.ultimateduels.libs.hikari.pool.ProxyLeakTaskFactory;
import com.ultimateduels.libs.hikari.util.ClockSource;
import com.ultimateduels.libs.hikari.util.ConcurrentBag;
import com.ultimateduels.libs.hikari.util.SuspendResumeLock;
import com.ultimateduels.libs.hikari.util.UtilityElf;
...
ConcurrentBag.IBagStateListener {
    ...
    private final long aliveBypassWindowMs = Long.getLong("com.ultimateduels.libs.hikari.aliveBypassWindowMs", TimeUnit.MILLISECONDS.toMillis(500L));
    private final long housekeepingPeriodMs = Long.getLong("com.ultimateduels.libs.hikari.housekeeping.periodMs", TimeUnit.SECONDS.toMillis(30L));
    ...
    public HikariPool(HikariConfig config) {
        ...
        this.addConnectionExecutor = UtilityElf.createThreadPoolExecutor(addConnectionQueue, this.poolName + " connection adder", threadFactory, (RejectedExecutionHandler)new UtilityElf.CustomDiscardPolicy());
        ...
        if (Boolean.getBoolean("com.ultimateduels.libs.hikari.blockUntilFilled") && config.getInitializationFailTimeout() > 1L) {
    ...
    private ScheduledExecutorService initializeHouseKeepingExecutorService() {
        if (this.config.getScheduledExecutor() == null) {
            ThreadFactory threadFactory = Optional.ofNullable(this.config.getThreadFactory()).orElseGet(() -> new UtilityElf.DefaultThreadFactory(this.poolName + " housekeeper"));
            ...
            try {
                ...
                if (idleTimeout > 0L && HikariPool.this.config.getMinimumIdle() < HikariPool.this.config.getMaximumPoolSize()) {
                    ...
                    for (PoolEntry entry : notInUse) {
                        if (maxToRemove <= 0 || ClockSource.elapsedMillis(entry.lastAccessed, now) <= idleTimeout || !HikariPool.this.connectionBag.reserve(entry)) continue;
            ...
            block8: {
                block7: {
                    ...
                    try {
                        ...
                        if (!added || this.loggingPrefix == null) break block7;
                    }
                    catch (Throwable throwable) {
